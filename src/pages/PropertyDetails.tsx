...
import { generatePropertyStructuredData, generateBreadcrumbStructuredData } from '@/utils/seo';
...
const PropertyDetails = () => {
  ...
  useEffect(() => {
    if (property) {
      const seoTitle = `${property.title} - ${property.location} | FindMyPG`;
      const seoDescription = `${property.description || 'Comfortable PG accommodation'} Located in ${property.location}. Starting from ₹${property.price_single}/month. Book now!`;
      const seoKeywords = `PG in ${property.location}, ${property.property_type}, paying guest ${property.location}, accommodation ${property.location}`;
      const seoImage = property.images?.[0] || 'https://lovable.dev/opengraph-image-p98pqg.png';
      ...
      if (ogUrl) {
        ogUrl.setAttribute('content', `${window.location.origin}/property/${property.id}`);
      }
      ...
      script.textContent = JSON.stringify([propertyStructuredData, breadcrumbData]);
    ...
    <div className="min-h-screen bg-white">
      ...
      <article itemScope itemType="https://schema.org/LodgingBusiness">
          ...
          <div className="max-w-7xl mx-auto py-4 flex items-center justify-between">
            <nav aria-label="Breadcrumb">
              <ol className="flex items-center space-x-2 text-sm text-gray-500">
                ...
                <li>/</li>
                <li><a href="/explore" className="hover:text-gray-700">Properties</a></li>
                <li>/</li>
            ...
            <div className="flex items-center space-x-2">
              <Button variant="ghost" size="sm" className="text-gray-600" aria-label="Share property">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          {/* Title Section with semantic markup */}
          <header className="mb-6">
            <h1 className="text-2xl md:text-3xl font-semibold text-gray-900 mb-2" itemProp="name">
              {property?.title}
            </h1>
            <div className="flex items-center text-gray-600 mb-4">
              {property?.rating && (
                <div className="flex items-center mr-4" itemProp="aggregateRating" itemScope itemType="https://schema.org/AggregateRating">
                  <Star className="h-4 w-4 text-black fill-current mr-1" />
                  <span className="font-medium text-black" itemProp="ratingValue">{property.rating}</span>
              ...
              <div className="flex items-center" itemProp="address" itemScope itemType="https://schema.org/PostalAddress">
          ...
          {/* Image Gallery with alt tags for accessibility */}
            ...
            <div className="grid grid-cols-4 gap-2 rounded-xl overflow-hidden h-[60vh] max-h-[500px]">
              ...
              <div className="col-span-4 md:col-span-2 relative">
                <img
                  ...
                  alt={`Main view of ${property?.title} in ${property?.location}`}
                ...
                {propertyImages.slice(1, 5).map((image, index) => (
                  <div key={index} className="relative">
                    <img
                      ...
                      alt={`${property?.title} interior view ${index + 2}`}
                      className="w-full h-full object-cover cursor-pointer"
                      onClick={() => setShowAllPhotos(true)}
                    />
